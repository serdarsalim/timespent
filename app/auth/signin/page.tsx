"use client";

import { signIn } from "next-auth/react";
import { APP_NAME } from "@/lib/branding";

export default function SignInPage() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-md space-y-8 text-center">
        <div>
          <h1 className="text-3xl font-bold text-foreground">{APP_NAME}</h1>
          <p className="mt-2 text-sm text-[color-mix(in_srgb,var(--foreground)_70%,transparent)]">
            Personal goal and productivity tracker
          </p>
        </div>

        <div className="rounded-3xl border border-[color-mix(in_srgb,var(--foreground)_12%,transparent)] bg-[color-mix(in_srgb,var(--foreground)_2%,transparent)] p-8">
          <h2 className="mb-6 text-xl font-semibold text-foreground">
            Sign up to save your data
          </h2>

          <div className="mb-6 space-y-3 text-left text-sm text-[color-mix(in_srgb,var(--foreground)_70%,transparent)]">
            <div className="flex items-start gap-2">
              <span className="mt-0.5">✓</span>
              <span>Save your goals and progress permanently</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5">✓</span>
              <span>Track productivity across all your devices</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5">✓</span>
              <span>Your data is private and secure</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5">⚠️</span>
              <span className="text-[color-mix(in_srgb,var(--foreground)_60%,transparent)]">
                Demo data is not saved - sign up to keep your work
              </span>
            </div>
          </div>

          <button
            onClick={() => signIn("google", { callbackUrl: "/" })}
            className="flex w-full items-center justify-center gap-3 rounded-full bg-foreground px-6 py-3 text-background transition hover:opacity-90"
          >
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.874 2.684-6.615z" fill="#4285F4"/>
              <path d="M9.003 18c2.43 0 4.467-.806 5.956-2.18L12.05 13.56c-.806.54-1.836.86-3.047.86-2.344 0-4.328-1.584-5.036-3.711H.96v2.332C2.438 15.983 5.482 18 9.003 18z" fill="#34A853"/>
              <path d="M3.964 10.712c-.18-.54-.282-1.117-.282-1.71 0-.593.102-1.17.282-1.71V4.96H.957C.347 6.175 0 7.55 0 9.002c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"/>
              <path d="M9.003 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.464.891 11.426 0 9.003 0 5.482 0 2.438 2.017.96 4.958L3.967 7.29c.708-2.127 2.692-3.71 5.036-3.71z" fill="#EA4335"/>
            </svg>
            Sign up with Google
          </button>

          <p className="mt-4 text-xs text-[color-mix(in_srgb,var(--foreground)_50%,transparent)]">
            Already have an account? You'll be signed in automatically.
          </p>
        </div>
      </div>
    </div>
  );
}
